...
print(B.shape)
print(C.shape)
result, valor, e, active, s, noise, n_adcs = cim(A.detach().to(device), B.detach().to(device), 1, 12, 8, 12, permutation = 'sorted', prints=True, perc=(K/1)*np.ones(1), num_sec=10, b_set = (8*np.ones((100,8))).tolist(), opt=1, add_noise=0, noise_gain=0, find_adc=0)
print(valor)
print(e)
print(active)
print(s)
