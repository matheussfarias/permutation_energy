...
def fc_to_cim(x, layer, v_ref = 1, d = 12, wq = 8, adc = 12, permutation = 'random', grid = False, perc=[68, 27], num_sec=100, b_set = None, opt=0, add_noise=0, noise_gain=0):
    ...
    if grid:
        for i in range(int(opt_grind*partial_result.shape[0])):
            print('Current: '+ str(i))
            t1=time.time()
            result, valor, energy_value,active,s,noise = cim(x[i][np.newaxis, :].detach(), layer.weight.detach().T, v_ref, d, wq, adc, permutation = permutation, prints=False,perc=perc, num_sec=num_sec, b_set = b_set, opt=opt, add_noise=add_noise, noise_gain=noise_gain)
            print(active)
